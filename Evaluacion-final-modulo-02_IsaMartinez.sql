...
SELECT DISTINCT title, rating AS clasification
FROM film
WHERE rating = "PG-13";
...
SELECT title, rating AS clasification
FROM film
...
INNER JOIN actor AS a ON i.actor_id = a.actor_id
ORDER BY first_name;
...
SELECT title, release_year -- Also select column 'release_year' for verification.
FROM film
...
*/
SELECT f.title, c.name -- Also select column 'category name' for verification.
FROM film AS f
...
INNER JOIN actor AS a ON recurring_actors.actor_id = a.actor_id
ORDER BY films_by_actor DESC;
...
/* All the data is in table film
Also select column 'length' for verification.*/
SELECT title, rating AS clasification, length -- Also select columns 'rating' and 'length' for verification.
FROM film
...
**DATEDIFF returns the difference between two date values --> DATEDIFF(end_date, start_date) AS result*/
		...
		-- Discarded option as it pulls duplicates (counting by rental_id):
		SELECT DISTINCT f.title, DATEDIFF(r.return_date, r.rental_date) AS rental_days
		FROM film AS f
...
-- Choosen option: it will not pull duplicates.
-- 1. Create subquery to obtain the inventory_if of those movies that were rented for more than 5 days.
SELECT inventory_id, DATEDIFF(return_date, rental_date) AS rental_days -- Also select column 'rental_days' for verification.
FROM rental
WHERE DATEDIFF(return_date, rental_date) >5
ORDER BY rental_days;

-- 2. Obtain the film_id of the movies wihtin the first subquery.
SELECT film_id
FROM inventory
...
-- 3. Obtain the title of the movies within the second subquery.
SELECT film.title
FROM film 
WHERE film_id IN
	(SELECT film_id
